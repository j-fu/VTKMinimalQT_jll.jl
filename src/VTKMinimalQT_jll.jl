...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import VTKMinimalQT_jll on Julia versions older than 1.3!")
end
    ...
    function dev_jll()
        ...
        mktempdir() do temp_env
            Pkg.activate(temp_env) do
                Pkg.develop("VTKMinimalQT_jll")
            end
        end
        # Create the override directory
        override_dir = joinpath(Pkg.devdir(), "VTKMinimalQT_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"VTKMinimalQT", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "VTKMinimalQT_jll", "src", "VTKMinimalQT_jll.jl"))
        @info("VTKMinimalQT_ll dev'ed out to /home/fuhrmann/Wias/work/julia/dev/VTKMinimalQT_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("cac670be-88ac-5a2d-850d-8ba8b403077c"))
platforms = [Pkg.Artifacts.unpack_platform(e, "VTKMinimalQT", artifacts_toml) for e in artifacts["VTKMinimalQT"]]
...
if best_platform === nothing
    @debug("Unable to load VTKMinimalQT; unsupported platform $(triplet(platform_key_abi()))")
else
