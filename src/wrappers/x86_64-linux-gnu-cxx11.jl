# Autogenerated wrapper script for VTKMinimalQT_jll for x86_64-linux-gnu-cxx11
...
const libvtkGUISupportQt_splitpath = ["lib", "libvtkGUISupportQt.so"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkGUISupportQt = "libvtkGUISupportQt.so.1"
...
const libvtkRenderingQt_splitpath = ["lib", "libvtkRenderingQt.so"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkRenderingQt = "libvtkRenderingQt.so.1"
...
const vtkProbeOpenGLVersion_splitpath = ["bin", "vtkProbeOpenGLVersion"]
