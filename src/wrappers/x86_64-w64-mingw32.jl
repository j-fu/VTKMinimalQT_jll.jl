# Autogenerated wrapper script for VTKMinimalQT_jll for x86_64-w64-mingw32
export libvtkCommonColor, libvtkCommonComputationalGeometry, libvtkCommonCore, libvtkCommonDataModel, libvtkCommonExecutionModel, libvtkCommonMath, libvtkCommonMisc, libvtkCommonSystem, libvtkCommonTransforms, libvtkDICOMParser, libvtkFiltersCore, libvtkFiltersExtraction, libvtkFiltersFlowPaths, libvtkFiltersGeneral, libvtkFiltersGeneric, libvtkFiltersGeometry, libvtkFiltersHybrid, libvtkFiltersHyperTree, libvtkFiltersImaging, libvtkFiltersModeling, libvtkFiltersParallel, libvtkFiltersParallelImaging, libvtkFiltersPoints, libvtkFiltersProgrammable, libvtkFiltersSMP, libvtkFiltersSelection, libvtkFiltersSources, libvtkFiltersStatistics, libvtkFiltersTexture, libvtkFiltersTopology, libvtkGUISupportQt, libvtkGUISupportQt, libvtkGUISupportQt, libvtkIOCore, libvtkIOImage, libvtkIOLegacy, libvtkIOMovie, libvtkIOOggTheora, libvtkIOXML, libvtkIOXMLParser, libvtkImagingColor, libvtkImagingCore, libvtkImagingFourier, libvtkImagingGeneral, libvtkImagingHybrid, libvtkImagingMath, libvtkImagingMorphological, libvtkImagingSources, libvtkImagingStatistics, libvtkImagingStencil, libvtkInteractionImage, libvtkInteractionStyle, libvtkInteractionWidgets, libvtkParallelCore, libvtkParallelDIY, libvtkRenderingAnnotation, libvtkRenderingContext2D, libvtkRenderingContextOpenGL2, libvtkRenderingCore, libvtkRenderingFreeType, libvtkRenderingImage, libvtkRenderingLOD, libvtkRenderingLabel, libvtkRenderingOpenGL2, libvtkRenderingQt, libvtkRenderingQt, libvtkRenderingQt, libvtkRenderingUI, libvtkRenderingVolume, libvtkRenderingVolumeOpenGL2, libvtkViewsCore, libvtkdoubleconversion, libvtkmetaio, libvtkogg, libvtkpugixml, libvtksys, libvtktheora, vtkProbeOpenGLVersion
...
const libvtkCommonColor_splitpath = ["bin", "libvtkCommonColor.dll"]
...
const libvtkCommonColor = "libvtkCommonColor.dll"


# Relative path to `libvtkCommonComputationalGeometry`
const libvtkCommonComputationalGeometry_splitpath = ["bin", "libvtkCommonComputationalGeometry.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkCommonComputationalGeometry = "libvtkCommonComputationalGeometry.dll"


# Relative path to `libvtkCommonCore`
const libvtkCommonCore_splitpath = ["bin", "libvtkCommonCore.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkCommonCore = "libvtkCommonCore.dll"


# Relative path to `libvtkCommonDataModel`
const libvtkCommonDataModel_splitpath = ["bin", "libvtkCommonDataModel.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkCommonDataModel = "libvtkCommonDataModel.dll"


# Relative path to `libvtkCommonExecutionModel`
const libvtkCommonExecutionModel_splitpath = ["bin", "libvtkCommonExecutionModel.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkCommonExecutionModel = "libvtkCommonExecutionModel.dll"


# Relative path to `libvtkCommonMath`
const libvtkCommonMath_splitpath = ["bin", "libvtkCommonMath.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkCommonMath = "libvtkCommonMath.dll"


# Relative path to `libvtkCommonMisc`
const libvtkCommonMisc_splitpath = ["bin", "libvtkCommonMisc.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkCommonMisc = "libvtkCommonMisc.dll"


# Relative path to `libvtkCommonSystem`
const libvtkCommonSystem_splitpath = ["bin", "libvtkCommonSystem.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkCommonSystem = "libvtkCommonSystem.dll"


# Relative path to `libvtkCommonTransforms`
const libvtkCommonTransforms_splitpath = ["bin", "libvtkCommonTransforms.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkCommonTransforms = "libvtkCommonTransforms.dll"


# Relative path to `libvtkDICOMParser`
const libvtkDICOMParser_splitpath = ["bin", "libvtkDICOMParser.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkDICOMParser = "libvtkDICOMParser.dll"


# Relative path to `libvtkFiltersCore`
const libvtkFiltersCore_splitpath = ["bin", "libvtkFiltersCore.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkFiltersCore = "libvtkFiltersCore.dll"


# Relative path to `libvtkFiltersExtraction`
const libvtkFiltersExtraction_splitpath = ["bin", "libvtkFiltersExtraction.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkFiltersExtraction = "libvtkFiltersExtraction.dll"


# Relative path to `libvtkFiltersFlowPaths`
const libvtkFiltersFlowPaths_splitpath = ["bin", "libvtkFiltersFlowPaths.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkFiltersFlowPaths = "libvtkFiltersFlowPaths.dll"


# Relative path to `libvtkFiltersGeneral`
const libvtkFiltersGeneral_splitpath = ["bin", "libvtkFiltersGeneral.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkFiltersGeneral = "libvtkFiltersGeneral.dll"


# Relative path to `libvtkFiltersGeneric`
const libvtkFiltersGeneric_splitpath = ["bin", "libvtkFiltersGeneric.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkFiltersGeneric = "libvtkFiltersGeneric.dll"


# Relative path to `libvtkFiltersGeometry`
const libvtkFiltersGeometry_splitpath = ["bin", "libvtkFiltersGeometry.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkFiltersGeometry = "libvtkFiltersGeometry.dll"


# Relative path to `libvtkFiltersHybrid`
const libvtkFiltersHybrid_splitpath = ["bin", "libvtkFiltersHybrid.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkFiltersHybrid = "libvtkFiltersHybrid.dll"


# Relative path to `libvtkFiltersHyperTree`
const libvtkFiltersHyperTree_splitpath = ["bin", "libvtkFiltersHyperTree.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkFiltersHyperTree = "libvtkFiltersHyperTree.dll"


# Relative path to `libvtkFiltersImaging`
const libvtkFiltersImaging_splitpath = ["bin", "libvtkFiltersImaging.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkFiltersImaging = "libvtkFiltersImaging.dll"


# Relative path to `libvtkFiltersModeling`
const libvtkFiltersModeling_splitpath = ["bin", "libvtkFiltersModeling.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkFiltersModeling = "libvtkFiltersModeling.dll"


# Relative path to `libvtkFiltersParallel`
const libvtkFiltersParallel_splitpath = ["bin", "libvtkFiltersParallel.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkFiltersParallel = "libvtkFiltersParallel.dll"


# Relative path to `libvtkFiltersParallelImaging`
const libvtkFiltersParallelImaging_splitpath = ["bin", "libvtkFiltersParallelImaging.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkFiltersParallelImaging = "libvtkFiltersParallelImaging.dll"


# Relative path to `libvtkFiltersPoints`
const libvtkFiltersPoints_splitpath = ["bin", "libvtkFiltersPoints.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkFiltersPoints = "libvtkFiltersPoints.dll"


# Relative path to `libvtkFiltersProgrammable`
const libvtkFiltersProgrammable_splitpath = ["bin", "libvtkFiltersProgrammable.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkFiltersProgrammable = "libvtkFiltersProgrammable.dll"


# Relative path to `libvtkFiltersSMP`
const libvtkFiltersSMP_splitpath = ["bin", "libvtkFiltersSMP.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkFiltersSMP = "libvtkFiltersSMP.dll"


# Relative path to `libvtkFiltersSelection`
const libvtkFiltersSelection_splitpath = ["bin", "libvtkFiltersSelection.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkFiltersSelection = "libvtkFiltersSelection.dll"


# Relative path to `libvtkFiltersSources`
const libvtkFiltersSources_splitpath = ["bin", "libvtkFiltersSources.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkFiltersSources = "libvtkFiltersSources.dll"


# Relative path to `libvtkFiltersStatistics`
const libvtkFiltersStatistics_splitpath = ["bin", "libvtkFiltersStatistics.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkFiltersStatistics = "libvtkFiltersStatistics.dll"


# Relative path to `libvtkFiltersTexture`
const libvtkFiltersTexture_splitpath = ["bin", "libvtkFiltersTexture.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkFiltersTexture = "libvtkFiltersTexture.dll"


# Relative path to `libvtkFiltersTopology`
const libvtkFiltersTopology_splitpath = ["bin", "libvtkFiltersTopology.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkFiltersTopology = "libvtkFiltersTopology.dll"


# Relative path to `libvtkGUISupportQt`
const libvtkGUISupportQt_splitpath = ["bin", "libvtkGUISupportQt.dll"]

# This will be filled out by __init__() for all products, as it must be done at runtime
libvtkGUISupportQt_path = ""

# libvtkGUISupportQt-specific global declaration
# This will be filled out by __init__()
libvtkGUISupportQt_handle = C_NULL

# This must be `const` so that we can use it with `ccall()`
const libvtkGUISupportQt = "libvtkGUISupportQt.dll"
...
const libvtkIOCore_splitpath = ["bin", "libvtkIOCore.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkIOCore = "libvtkIOCore.dll"


# Relative path to `libvtkIOImage`
const libvtkIOImage_splitpath = ["bin", "libvtkIOImage.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkIOImage = "libvtkIOImage.dll"


# Relative path to `libvtkIOLegacy`
const libvtkIOLegacy_splitpath = ["bin", "libvtkIOLegacy.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkIOLegacy = "libvtkIOLegacy.dll"


# Relative path to `libvtkIOMovie`
const libvtkIOMovie_splitpath = ["bin", "libvtkIOMovie.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkIOMovie = "libvtkIOMovie.dll"


# Relative path to `libvtkIOOggTheora`
const libvtkIOOggTheora_splitpath = ["bin", "libvtkIOOggTheora.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkIOOggTheora = "libvtkIOOggTheora.dll"


# Relative path to `libvtkIOXML`
const libvtkIOXML_splitpath = ["bin", "libvtkIOXML.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkIOXML = "libvtkIOXML.dll"


# Relative path to `libvtkIOXMLParser`
const libvtkIOXMLParser_splitpath = ["bin", "libvtkIOXMLParser.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkIOXMLParser = "libvtkIOXMLParser.dll"


# Relative path to `libvtkImagingColor`
const libvtkImagingColor_splitpath = ["bin", "libvtkImagingColor.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkImagingColor = "libvtkImagingColor.dll"


# Relative path to `libvtkImagingCore`
const libvtkImagingCore_splitpath = ["bin", "libvtkImagingCore.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkImagingCore = "libvtkImagingCore.dll"


# Relative path to `libvtkImagingFourier`
const libvtkImagingFourier_splitpath = ["bin", "libvtkImagingFourier.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkImagingFourier = "libvtkImagingFourier.dll"


# Relative path to `libvtkImagingGeneral`
const libvtkImagingGeneral_splitpath = ["bin", "libvtkImagingGeneral.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkImagingGeneral = "libvtkImagingGeneral.dll"


# Relative path to `libvtkImagingHybrid`
const libvtkImagingHybrid_splitpath = ["bin", "libvtkImagingHybrid.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkImagingHybrid = "libvtkImagingHybrid.dll"


# Relative path to `libvtkImagingMath`
const libvtkImagingMath_splitpath = ["bin", "libvtkImagingMath.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkImagingMath = "libvtkImagingMath.dll"


# Relative path to `libvtkImagingMorphological`
const libvtkImagingMorphological_splitpath = ["bin", "libvtkImagingMorphological.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkImagingMorphological = "libvtkImagingMorphological.dll"


# Relative path to `libvtkImagingSources`
const libvtkImagingSources_splitpath = ["bin", "libvtkImagingSources.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkImagingSources = "libvtkImagingSources.dll"


# Relative path to `libvtkImagingStatistics`
const libvtkImagingStatistics_splitpath = ["bin", "libvtkImagingStatistics.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkImagingStatistics = "libvtkImagingStatistics.dll"


# Relative path to `libvtkImagingStencil`
const libvtkImagingStencil_splitpath = ["bin", "libvtkImagingStencil.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkImagingStencil = "libvtkImagingStencil.dll"


# Relative path to `libvtkInteractionImage`
const libvtkInteractionImage_splitpath = ["bin", "libvtkInteractionImage.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkInteractionImage = "libvtkInteractionImage.dll"


# Relative path to `libvtkInteractionStyle`
const libvtkInteractionStyle_splitpath = ["bin", "libvtkInteractionStyle.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkInteractionStyle = "libvtkInteractionStyle.dll"


# Relative path to `libvtkInteractionWidgets`
const libvtkInteractionWidgets_splitpath = ["bin", "libvtkInteractionWidgets.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkInteractionWidgets = "libvtkInteractionWidgets.dll"


# Relative path to `libvtkParallelCore`
const libvtkParallelCore_splitpath = ["bin", "libvtkParallelCore.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkParallelCore = "libvtkParallelCore.dll"


# Relative path to `libvtkParallelDIY`
const libvtkParallelDIY_splitpath = ["bin", "libvtkParallelDIY.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkParallelDIY = "libvtkParallelDIY.dll"


# Relative path to `libvtkRenderingAnnotation`
const libvtkRenderingAnnotation_splitpath = ["bin", "libvtkRenderingAnnotation.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkRenderingAnnotation = "libvtkRenderingAnnotation.dll"


# Relative path to `libvtkRenderingContext2D`
const libvtkRenderingContext2D_splitpath = ["bin", "libvtkRenderingContext2D.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkRenderingContext2D = "libvtkRenderingContext2D.dll"


# Relative path to `libvtkRenderingContextOpenGL2`
const libvtkRenderingContextOpenGL2_splitpath = ["bin", "libvtkRenderingContextOpenGL2.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkRenderingContextOpenGL2 = "libvtkRenderingContextOpenGL2.dll"


# Relative path to `libvtkRenderingCore`
const libvtkRenderingCore_splitpath = ["bin", "libvtkRenderingCore.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkRenderingCore = "libvtkRenderingCore.dll"


# Relative path to `libvtkRenderingFreeType`
const libvtkRenderingFreeType_splitpath = ["bin", "libvtkRenderingFreeType.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkRenderingFreeType = "libvtkRenderingFreeType.dll"


# Relative path to `libvtkRenderingImage`
const libvtkRenderingImage_splitpath = ["bin", "libvtkRenderingImage.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkRenderingImage = "libvtkRenderingImage.dll"


# Relative path to `libvtkRenderingLOD`
const libvtkRenderingLOD_splitpath = ["bin", "libvtkRenderingLOD.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkRenderingLOD = "libvtkRenderingLOD.dll"


# Relative path to `libvtkRenderingLabel`
const libvtkRenderingLabel_splitpath = ["bin", "libvtkRenderingLabel.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkRenderingLabel = "libvtkRenderingLabel.dll"


# Relative path to `libvtkRenderingOpenGL2`
const libvtkRenderingOpenGL2_splitpath = ["bin", "libvtkRenderingOpenGL2.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkRenderingOpenGL2 = "libvtkRenderingOpenGL2.dll"


# Relative path to `libvtkRenderingQt`
const libvtkRenderingQt_splitpath = ["bin", "libvtkRenderingQt.dll"]

# This will be filled out by __init__() for all products, as it must be done at runtime
libvtkRenderingQt_path = ""

# libvtkRenderingQt-specific global declaration
# This will be filled out by __init__()
libvtkRenderingQt_handle = C_NULL

# This must be `const` so that we can use it with `ccall()`
const libvtkRenderingQt = "libvtkRenderingQt.dll"
...
const libvtkRenderingUI_splitpath = ["bin", "libvtkRenderingUI.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkRenderingUI = "libvtkRenderingUI.dll"


# Relative path to `libvtkRenderingVolume`
const libvtkRenderingVolume_splitpath = ["bin", "libvtkRenderingVolume.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkRenderingVolume = "libvtkRenderingVolume.dll"


# Relative path to `libvtkRenderingVolumeOpenGL2`
const libvtkRenderingVolumeOpenGL2_splitpath = ["bin", "libvtkRenderingVolumeOpenGL2.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkRenderingVolumeOpenGL2 = "libvtkRenderingVolumeOpenGL2.dll"


# Relative path to `libvtkViewsCore`
const libvtkViewsCore_splitpath = ["bin", "libvtkViewsCore.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkViewsCore = "libvtkViewsCore.dll"


# Relative path to `libvtkdoubleconversion`
const libvtkdoubleconversion_splitpath = ["bin", "libvtkdoubleconversion.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkdoubleconversion = "libvtkdoubleconversion.dll"


# Relative path to `libvtkmetaio`
const libvtkmetaio_splitpath = ["bin", "libvtkmetaio.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkmetaio = "libvtkmetaio.dll"


# Relative path to `libvtkogg`
const libvtkogg_splitpath = ["bin", "libvtkogg.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkogg = "libvtkogg.dll"


# Relative path to `libvtkpugixml`
const libvtkpugixml_splitpath = ["bin", "libvtkpugixml.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtkpugixml = "libvtkpugixml.dll"


# Relative path to `libvtksys`
const libvtksys_splitpath = ["bin", "libvtksys.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtksys = "libvtksys.dll"


# Relative path to `libvtktheora`
const libvtktheora_splitpath = ["bin", "libvtktheora.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libvtktheora = "libvtktheora.dll"


# Relative path to `vtkProbeOpenGLVersion`
const vtkProbeOpenGLVersion_splitpath = ["bin", "vtkProbeOpenGLVersion.exe"]

# This will be filled out by __init__() for all products, as it must be done at runtime
vtkProbeOpenGLVersion_path = ""

# vtkProbeOpenGLVersion-specific global declaration
function vtkProbeOpenGLVersion(f::Function; adjust_PATH::Bool = true, adjust_LIBPATH::Bool = true)
    global PATH, LIBPATH
...
function __init__()
    ...
    foreach(p -> append!(PATH_list, p), (GLFW_jll.PATH_list, GLEW_jll.PATH_list, Qt_jll.PATH_list, Libtiff_jll.PATH_list, JpegTurbo_jll.PATH_list, libpng_jll.PATH_list, Zlib_jll.PATH_list, FreeType2_jll.PATH_list, Expat_jll.PATH_list, Lz4_jll.PATH_list, XZ_jll.PATH_list,))
    foreach(p -> append!(LIBPATH_list, p), (GLFW_jll.LIBPATH_list, GLEW_jll.LIBPATH_list, Qt_jll.LIBPATH_list, Libtiff_jll.LIBPATH_list, JpegTurbo_jll.LIBPATH_list, libpng_jll.LIBPATH_list, Zlib_jll.LIBPATH_list, FreeType2_jll.LIBPATH_list, Expat_jll.LIBPATH_list, Lz4_jll.LIBPATH_list, XZ_jll.LIBPATH_list,))

    global libvtkCommonColor_path = normpath(joinpath(artifact_dir, libvtkCommonColor_splitpath...))

    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkCommonColor_handle = dlopen(libvtkCommonColor_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkCommonComputationalGeometry_handle = dlopen(libvtkCommonComputationalGeometry_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkCommonCore_handle = dlopen(libvtkCommonCore_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkCommonDataModel_handle = dlopen(libvtkCommonDataModel_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkCommonExecutionModel_handle = dlopen(libvtkCommonExecutionModel_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkCommonMath_handle = dlopen(libvtkCommonMath_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkCommonMisc_handle = dlopen(libvtkCommonMisc_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkCommonSystem_handle = dlopen(libvtkCommonSystem_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkCommonTransforms_handle = dlopen(libvtkCommonTransforms_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkDICOMParser_handle = dlopen(libvtkDICOMParser_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkFiltersCore_handle = dlopen(libvtkFiltersCore_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkFiltersExtraction_handle = dlopen(libvtkFiltersExtraction_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkFiltersFlowPaths_handle = dlopen(libvtkFiltersFlowPaths_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkFiltersGeneral_handle = dlopen(libvtkFiltersGeneral_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkFiltersGeneric_handle = dlopen(libvtkFiltersGeneric_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkFiltersGeometry_handle = dlopen(libvtkFiltersGeometry_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkFiltersHybrid_handle = dlopen(libvtkFiltersHybrid_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkFiltersHyperTree_handle = dlopen(libvtkFiltersHyperTree_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkFiltersImaging_handle = dlopen(libvtkFiltersImaging_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkFiltersModeling_handle = dlopen(libvtkFiltersModeling_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkFiltersParallel_handle = dlopen(libvtkFiltersParallel_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkFiltersParallelImaging_handle = dlopen(libvtkFiltersParallelImaging_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkFiltersPoints_handle = dlopen(libvtkFiltersPoints_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkFiltersProgrammable_handle = dlopen(libvtkFiltersProgrammable_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkFiltersSMP_handle = dlopen(libvtkFiltersSMP_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkFiltersSelection_handle = dlopen(libvtkFiltersSelection_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkFiltersSources_handle = dlopen(libvtkFiltersSources_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkFiltersStatistics_handle = dlopen(libvtkFiltersStatistics_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkFiltersTexture_handle = dlopen(libvtkFiltersTexture_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkFiltersTopology_handle = dlopen(libvtkFiltersTopology_path, RTLD_LAZY | RTLD_DEEPBIND)
    push!(LIBPATH_list, dirname(libvtkFiltersTopology_path))

    global libvtkGUISupportQt_path = normpath(joinpath(artifact_dir, libvtkGUISupportQt_splitpath...))

    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkGUISupportQt_handle = dlopen(libvtkGUISupportQt_path, RTLD_LAZY | RTLD_DEEPBIND)
    push!(LIBPATH_list, dirname(libvtkGUISupportQt_path))
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkIOCore_handle = dlopen(libvtkIOCore_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkIOImage_handle = dlopen(libvtkIOImage_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkIOLegacy_handle = dlopen(libvtkIOLegacy_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkIOMovie_handle = dlopen(libvtkIOMovie_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkIOOggTheora_handle = dlopen(libvtkIOOggTheora_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkIOXML_handle = dlopen(libvtkIOXML_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkIOXMLParser_handle = dlopen(libvtkIOXMLParser_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkImagingColor_handle = dlopen(libvtkImagingColor_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkImagingCore_handle = dlopen(libvtkImagingCore_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkImagingFourier_handle = dlopen(libvtkImagingFourier_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkImagingGeneral_handle = dlopen(libvtkImagingGeneral_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkImagingHybrid_handle = dlopen(libvtkImagingHybrid_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkImagingMath_handle = dlopen(libvtkImagingMath_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkImagingMorphological_handle = dlopen(libvtkImagingMorphological_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkImagingSources_handle = dlopen(libvtkImagingSources_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkImagingStatistics_handle = dlopen(libvtkImagingStatistics_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkImagingStencil_handle = dlopen(libvtkImagingStencil_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkInteractionImage_handle = dlopen(libvtkInteractionImage_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkInteractionStyle_handle = dlopen(libvtkInteractionStyle_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkInteractionWidgets_handle = dlopen(libvtkInteractionWidgets_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkParallelCore_handle = dlopen(libvtkParallelCore_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkParallelDIY_handle = dlopen(libvtkParallelDIY_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkRenderingAnnotation_handle = dlopen(libvtkRenderingAnnotation_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkRenderingContext2D_handle = dlopen(libvtkRenderingContext2D_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkRenderingContextOpenGL2_handle = dlopen(libvtkRenderingContextOpenGL2_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkRenderingCore_handle = dlopen(libvtkRenderingCore_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkRenderingFreeType_handle = dlopen(libvtkRenderingFreeType_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkRenderingImage_handle = dlopen(libvtkRenderingImage_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkRenderingLOD_handle = dlopen(libvtkRenderingLOD_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkRenderingLabel_handle = dlopen(libvtkRenderingLabel_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkRenderingOpenGL2_handle = dlopen(libvtkRenderingOpenGL2_path, RTLD_LAZY | RTLD_DEEPBIND)
    push!(LIBPATH_list, dirname(libvtkRenderingOpenGL2_path))

    global libvtkRenderingQt_path = normpath(joinpath(artifact_dir, libvtkRenderingQt_splitpath...))

    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkRenderingQt_handle = dlopen(libvtkRenderingQt_path, RTLD_LAZY | RTLD_DEEPBIND)
    push!(LIBPATH_list, dirname(libvtkRenderingQt_path))
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkRenderingUI_handle = dlopen(libvtkRenderingUI_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkRenderingVolume_handle = dlopen(libvtkRenderingVolume_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkRenderingVolumeOpenGL2_handle = dlopen(libvtkRenderingVolumeOpenGL2_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkViewsCore_handle = dlopen(libvtkViewsCore_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkdoubleconversion_handle = dlopen(libvtkdoubleconversion_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkmetaio_handle = dlopen(libvtkmetaio_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkogg_handle = dlopen(libvtkogg_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtkpugixml_handle = dlopen(libvtkpugixml_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtksys_handle = dlopen(libvtksys_path, RTLD_LAZY | RTLD_DEEPBIND)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libvtktheora_handle = dlopen(libvtktheora_path, RTLD_LAZY | RTLD_DEEPBIND)
    push!(LIBPATH_list, dirname(libvtktheora_path))

    global vtkProbeOpenGLVersion_path = normpath(joinpath(artifact_dir, vtkProbeOpenGLVersion_splitpath...))

    push!(PATH_list, dirname(vtkProbeOpenGLVersion_path))
